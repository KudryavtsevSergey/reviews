...
if ($bAllowAccess)
{
	...
	if (check_bitrix_sessid() && (!empty($_REQUEST["iblock_submit"]) || !empty($_REQUEST["iblock_apply"])))
	{
		...
		foreach ($arParams["PROPERTY_CODES_REQUIRED"] as $key => $propertyID)
		{
			...
			{
			    if (empty($propertyValue) || strlen($propertyValue) == 0){
                    $bError = true;
                }
				/*$len = 0;
		...
		if (empty($arResult["ERRORS"]))
		{
			...
			{
			    if(!empty($arParams["ELEMENT_BINDING_NAME"]) && !empty($arParams["ID_LINK_PRODUCT"])) {
                    $arUpdateValues["PROPERTY_VALUES"][$arParams["ELEMENT_BINDING_NAME"]] = $arParams["ID_LINK_PRODUCT"];
                }
                $arUpdateValues["NAME"] = $_REQUEST["PROPERTY"][$GLOBALS["ID_FIO_REVIEW"]][0].', '.date($DB->DateFormatToPHP(CSite::GetDateFormat("SHORT")));
                $arUpdateValues["CODE"] = Cutil::translit($_REQUEST["PROPERTY"][$GLOBALS["ID_FIO_REVIEW"]][0],"ru",NULL).'_'.time();
